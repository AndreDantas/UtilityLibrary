...
public class EllipseCollisionTest
{
    private float[] innerPolygonCoef;
    private float[] outerPolygonCoef;
    private int maxIterations;

    private bool iterate(float x, float y, float c0x, float c0y, float c2x, float c2y, float rr)
    {
    ...
    public bool EllipseEllipseCollide(float x0, float y0, float wx0, float wy0, float hw0,
           float x1, float y1, float wx1, float wy1, float hw1)
    {
    ...
    public bool EllipseEllipseCollide(float x0, float y0, float w0, float h0, float x1, float y1, float w1, float h1)
    {

        float x = Mathf.Abs(x1 - x0) * h1;
        float y = Mathf.Abs(y1 - y0) * w1;
        w0 *= h1;
        h0 *= w1;
        float r = w1 * h1;
    ...
    public bool EllipseEllipseCollide(Ellipse e1, Ellipse e2)
    {
        return EllipseEllipseCollide(e1.Center.x, e1.Center.y, e1.Width / 2f, e1.Height / 2f, e2.Center.x, e2.Center.y, e2.Width / 2f, e2.Height / 2f);
    }
    ...
    public bool EllipseCircleCollide(float x0, float y0, float w, float h, float x1, float y1, float r)
    {
    ...
    public bool EllipseCircleCollide(Ellipse e, Vector2 circleCenter, float circleRadius)
    {
        return EllipseCircleCollide(e.Center.x, e.Center.y, e.Width / 2f, e.Height / 2f, circleCenter.x, circleCenter.y, circleRadius);
    }

    public EllipseCollisionTest(int maxIterations)
    {
        ...
        {
            int numNodes = 4 << t;
            innerPolygonCoef[t] = 0.5f / Mathf.Cos(4 * Mathf.Acos(0.0f) / numNodes);
            outerPolygonCoef[t] = 0.5f / (Mathf.Cos(2 * Mathf.Acos(0.0f) / numNodes) * Mathf.Cos(2 * Mathf.Acos(0.0f) / numNodes));
        }
    }
